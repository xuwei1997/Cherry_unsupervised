...
if __name__ == '__main__':
    ...
    # x_train, x_val, x_test = X[:1000], X[1000:1500], X[1500:2000]
    # y_train, y_val, y_test = Y[:1000], Y[1000:1500], Y[1500:2000]
    ...
    siamese.compile(loss=loss(T=0.1), optimizer="Adam", metrics=["mse"])
    siamese.summary()
    ...
    history = siamese.fit_generator(minibatches(x_train, y_train, batch_size=b_size, p=p),
                                    ...
                                    epochs=epochs,
                                    callbacks = [checkpoint,logging,loss_history]
                                    )
    # print(history)

    # results = siamese.evaluate([x_test_1, x_test_2], labels_test)
    results = siamese.evaluate_generator(minibatches(x_test, y_test, batch_size=b_evaluate_size, p=p),steps=len(y_test) // b_evaluate_size)
